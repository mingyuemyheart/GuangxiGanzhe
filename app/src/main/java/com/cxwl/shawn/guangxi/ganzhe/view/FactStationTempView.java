 ...
 * 蔗田监测-田间实况--站点查询-气温
 */
public class FactStationTempView extends View {
	...
	public FactStationTempView(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactStationTempView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactStationTempView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setData(List<FactDto> lastStations, List<FactDto> currentStations) {
		if (!currentStations.isEmpty()) {
			...
			for (int i = 0; i < currentStations.size(); i++) {
				...
				if (!TextUtils.isEmpty(dto.TEMP) && !TextUtils.equals(dto.TEMP, "--")) {
					max = Float.valueOf(dto.TEMP);
					min = Float.valueOf(dto.TEMP);
				}
			}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < lastStations.size(); i++) {
			...
			float value = minValue;
			if (!TextUtils.isEmpty(dto.TEMPL) && !TextUtils.equals(dto.TEMPL, "--")) {
				value = Float.valueOf(dto.TEMPL);
			}
			dto.y = chartH-chartH*Math.abs(value)/(Math.abs(maxValue)+Math.abs(minValue)) + topMargin;
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.TEMP) && !TextUtils.equals(dto.TEMP, "--")) {
				...
				if (value != 0) {
					float tempWidth = textP.measureText(dto.TEMP+"");
					canvas.drawText(dto.TEMP+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
		...
		for (int i = 0; i < lastStations.size(); i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.TEMPL) && !TextUtils.equals(dto.TEMPL, "--")) {
				float value = Float.valueOf(dto.TEMPL);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.TEMPL+"");
					canvas.drawText(dto.TEMPL+"", dto.x-tempWidth/2, dto.y+(int)CommonUtil.dip2px(mContext, 10), textP);
				}
			}
		}
