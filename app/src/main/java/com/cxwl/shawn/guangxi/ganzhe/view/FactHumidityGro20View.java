 ...
 */
public class FactHumidityGro20View extends View {
	...
	public FactHumidityGro20View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactHumidityGro20View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactHumidityGro20View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.grohumidity20) && !TextUtils.equals(dto.grohumidity20, "--")) {
				value = Float.valueOf(dto.grohumidity20);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.grohumidity20) && !TextUtils.equals(dto.grohumidity20, "--")) {
				float value = Float.valueOf(dto.grohumidity20);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.grohumidity20+"");
					canvas.drawText(dto.grohumidity20+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
