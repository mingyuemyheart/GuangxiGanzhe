 ...
 * 蔗田监测-田间实况--小气候查询-湿度
 */
public class FactHumidity150View extends View {
	...
	public FactHumidity150View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactHumidity150View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactHumidity150View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.humidity150) && !TextUtils.equals(dto.humidity150, "--")) {
				value = Float.valueOf(dto.humidity150);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.humidity150) && !TextUtils.equals(dto.humidity150, "--")) {
				float value = Float.valueOf(dto.humidity150);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.humidity150+"");
					canvas.drawText(dto.humidity150+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
