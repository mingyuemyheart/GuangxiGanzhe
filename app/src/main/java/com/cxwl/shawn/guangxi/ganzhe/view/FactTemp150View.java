 ...
 */
public class FactTemp150View extends View {
	...
	public FactTemp150View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactTemp150View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactTemp150View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setData(List<FactDto> dataList) {
		if (!dataList.isEmpty()) {
			...
			for (int i = 0; i < tempList.size(); i++) {
				...
				if (!TextUtils.isEmpty(dto.temp150) && !TextUtils.equals(dto.temp150, "--")) {
					max = Float.valueOf(dto.temp150);
					min = Float.valueOf(dto.temp150);
				}
			}
			...
			for (int i = 0; i < tempList.size(); i++) {
				FactDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.temp150) && !TextUtils.equals(dto.temp150, "--")) {
					if (maxValue <= Float.valueOf(dto.temp150)) {
						maxValue = Float.valueOf(dto.temp150);
					}
					if (minValue >= Float.valueOf(dto.temp150)) {
						minValue = Float.valueOf(dto.temp150);
					}
				}
			}
...
//				minValue = minValue-itemDivider;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.temp150) && !TextUtils.equals(dto.temp150, "--")) {
				value = Float.valueOf(dto.temp150);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.temp150) && !TextUtils.equals(dto.temp150, "--")) {
				float value = Float.valueOf(dto.temp150);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.temp150+"");
					canvas.drawText(dto.temp150+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
