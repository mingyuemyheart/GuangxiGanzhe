 ...
 */
public class FactTemp60View extends View {
	...
	public FactTemp60View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactTemp60View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactTemp60View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setData(List<FactDto> dataList) {
		if (!dataList.isEmpty()) {
			...
			for (int i = 0; i < tempList.size(); i++) {
				...
				if (!TextUtils.isEmpty(dto.temp60) && !TextUtils.equals(dto.temp60, "--")) {
					max = Float.valueOf(dto.temp60);
					min = Float.valueOf(dto.temp60);
				}
			}
			...
			for (int i = 0; i < tempList.size(); i++) {
				FactDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.temp60) && !TextUtils.equals(dto.temp60, "--")) {
					if (maxValue <= Float.valueOf(dto.temp60)) {
						maxValue = Float.valueOf(dto.temp60);
					}
					if (minValue >= Float.valueOf(dto.temp60)) {
						minValue = Float.valueOf(dto.temp60);
					}
				}
			}
...
//				minValue = minValue-itemDivider;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.temp60) && !TextUtils.equals(dto.temp60, "--")) {
				value = Float.valueOf(dto.temp60);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.temp60) && !TextUtils.equals(dto.temp60, "--")) {
				float value = Float.valueOf(dto.temp60);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.temp60+"");
					canvas.drawText(dto.temp60+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
