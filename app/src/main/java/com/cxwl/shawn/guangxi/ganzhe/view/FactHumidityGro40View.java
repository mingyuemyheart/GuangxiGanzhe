...
/**
 * 蔗田监测-田间实况--小气候查询-土壤含水量
 */
public class FactHumidityGro40View extends View {
	...
	public FactHumidityGro40View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactHumidityGro40View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactHumidityGro40View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.grohumidity40) && !TextUtils.equals(dto.grohumidity40, "--")) {
				value = Float.valueOf(dto.grohumidity40);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.grohumidity40) && !TextUtils.equals(dto.grohumidity40, "--")) {
				float value = Float.valueOf(dto.grohumidity40);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.grohumidity40+"");
					canvas.drawText(dto.grohumidity40+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
