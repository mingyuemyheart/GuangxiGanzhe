 ...
 */
public class FactTempGro50View extends View {
	...
	public FactTempGro50View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactTempGro50View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactTempGro50View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setData(List<FactDto> dataList) {
		if (!dataList.isEmpty()) {
			...
			for (int i = 0; i < tempList.size(); i++) {
				...
				if (!TextUtils.isEmpty(dto.grotemp50) && !TextUtils.equals(dto.grotemp50, "--")) {
					max = Float.valueOf(dto.grotemp50);
					min = Float.valueOf(dto.grotemp50);
				}
			}
			...
			for (int i = 0; i < tempList.size(); i++) {
				FactDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.grotemp50) && !TextUtils.equals(dto.grotemp50, "--")) {
					if (maxValue <= Float.valueOf(dto.grotemp50)) {
						maxValue = Float.valueOf(dto.grotemp50);
					}
					if (minValue >= Float.valueOf(dto.grotemp50)) {
						minValue = Float.valueOf(dto.grotemp50);
					}
				}
			}
...
//				minValue = minValue-itemDivider;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.grotemp50) && !TextUtils.equals(dto.grotemp50, "--")) {
				value = Float.valueOf(dto.grotemp50);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.grotemp50) && !TextUtils.equals(dto.grotemp50, "--")) {
				float value = Float.valueOf(dto.grotemp50);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.grotemp50+"");
					canvas.drawText(dto.grotemp50+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
