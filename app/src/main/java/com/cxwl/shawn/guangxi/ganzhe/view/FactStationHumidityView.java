 ...
 * 蔗田监测-田间实况--站点查询-相对湿度
 */
public class FactStationHumidityView extends View {
	...
	public FactStationHumidityView(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactStationHumidityView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactStationHumidityView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.RELA_HUMI) && !TextUtils.equals(dto.RELA_HUMI, "--")) {
				value = Float.valueOf(dto.RELA_HUMI);
			}
		...
		for (int i = 0; i < lastStations.size(); i++) {
			...
			float value = minValue;
			if (!TextUtils.isEmpty(dto.RELA_HUMIL) && !TextUtils.equals(dto.RELA_HUMIL, "--")) {
				value = Float.valueOf(dto.RELA_HUMIL);
			}
			dto.y = chartH-chartH*Math.abs(value)/(Math.abs(maxValue)+Math.abs(minValue)) + topMargin;
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.RELA_HUMI) && !TextUtils.equals(dto.RELA_HUMI, "--")) {
				float value = Float.valueOf(dto.RELA_HUMI);
				if (value != 0) {
					float RELA_HUMIWidth = textP.measureText(dto.RELA_HUMI+"");
					canvas.drawText(dto.RELA_HUMI+"", dto.x-RELA_HUMIWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.RELA_HUMIL) && !TextUtils.equals(dto.RELA_HUMIL, "--")) {
				float value = Float.valueOf(dto.RELA_HUMIL);
				if (value != 0) {
					float RELA_HUMIWidth = textP.measureText(dto.RELA_HUMIL+"");
					canvas.drawText(dto.RELA_HUMIL+"", dto.x-RELA_HUMIWidth/2, dto.y+(int)CommonUtil.dip2px(mContext, 10), textP);
				}
			}
		}
