 ...
 */
public class FactTemp300View extends View {
	...
	public FactTemp300View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactTemp300View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactTemp300View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setData(List<FactDto> dataList) {
		if (!dataList.isEmpty()) {
			...
			for (int i = 0; i < tempList.size(); i++) {
				...
				if (!TextUtils.isEmpty(dto.temp300) && !TextUtils.equals(dto.temp300, "--")) {
					max = Float.valueOf(dto.temp300);
					min = Float.valueOf(dto.temp300);
				}
			}
			...
			for (int i = 0; i < tempList.size(); i++) {
				FactDto dto = tempList.get(i);
				if (!TextUtils.isEmpty(dto.temp300) && !TextUtils.equals(dto.temp300, "--")) {
					if (maxValue <= Float.valueOf(dto.temp300)) {
						maxValue = Float.valueOf(dto.temp300);
					}
					if (minValue >= Float.valueOf(dto.temp300)) {
						minValue = Float.valueOf(dto.temp300);
					}
				}
			}
...
//				minValue = minValue-itemDivider;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.temp300) && !TextUtils.equals(dto.temp300, "--")) {
				value = Float.valueOf(dto.temp300);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.temp300) && !TextUtils.equals(dto.temp300, "--")) {
				float value = Float.valueOf(dto.temp300);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.temp300+"");
					canvas.drawText(dto.temp300+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
