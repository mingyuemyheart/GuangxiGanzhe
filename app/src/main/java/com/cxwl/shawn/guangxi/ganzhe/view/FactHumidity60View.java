 ...
 * 蔗田监测-田间实况--小气候查询-湿度
 */
public class FactHumidity60View extends View {
	...
	public FactHumidity60View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactHumidity60View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactHumidity60View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.humidity60) && !TextUtils.equals(dto.humidity60, "--")) {
				value = Float.valueOf(dto.humidity60);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.humidity60) && !TextUtils.equals(dto.humidity60, "--")) {
				float value = Float.valueOf(dto.humidity60);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.humidity60+"");
					canvas.drawText(dto.humidity60+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
