 ...
 * 蔗田监测-田间实况--小气候查询-湿度
 */
public class FactHumidity300View extends View {
	...
	public FactHumidity300View(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactHumidity300View(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactHumidity300View(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.humidity300) && !TextUtils.equals(dto.humidity300, "--")) {
				value = Float.valueOf(dto.humidity300);
			}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.humidity300) && !TextUtils.equals(dto.humidity300, "--")) {
				float value = Float.valueOf(dto.humidity300);
				if (value != 0) {
					float tempWidth = textP.measureText(dto.humidity300+"");
					canvas.drawText(dto.humidity300+"", dto.x-tempWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
