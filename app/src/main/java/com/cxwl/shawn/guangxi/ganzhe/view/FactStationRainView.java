...
/**
 * 蔗田监测-田间实况--站点查询-小时累计雨量
 */
public class FactStationRainView extends View {
	...
	public FactStationRainView(Context context) {
		super(context);
		mContext = context;
		init();
	}

	public FactStationRainView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}

	public FactStationRainView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setData(List<FactDto> lastStations, List<FactDto> currentStations) {
		if (!currentStations.isEmpty()) {
			...
			for (int i = 0; i < currentStations.size(); i++) {
				...
				if (!TextUtils.isEmpty(dto.BUCKET_ACC_RAIN_COUNT) && !TextUtils.equals(dto.BUCKET_ACC_RAIN_COUNT, "--")) {
					max = Float.valueOf(dto.BUCKET_ACC_RAIN_COUNT);
				}
			}
			...
			for (int i = 0; i < currentStations.size(); i++) {
				FactDto dto = currentStations.get(i);
				if (!TextUtils.isEmpty(dto.BUCKET_ACC_RAIN_COUNT) && !TextUtils.equals(dto.BUCKET_ACC_RAIN_COUNT, "--")) {
					if (maxValue <= Float.valueOf(dto.BUCKET_ACC_RAIN_COUNT)) {
						maxValue = Float.valueOf(dto.BUCKET_ACC_RAIN_COUNT);
					}
				}
			}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!TextUtils.isEmpty(dto.BUCKET_ACC_RAIN_COUNT) && !TextUtils.equals(dto.BUCKET_ACC_RAIN_COUNT, "--")) {
				value = Float.valueOf(dto.BUCKET_ACC_RAIN_COUNT);
			}
		...
		for (int i = 0; i < lastStations.size(); i++) {
			...
			float value = minValue;
			if (!TextUtils.isEmpty(dto.BUCKET_ACC_RAIN_COUNTL) && !TextUtils.equals(dto.BUCKET_ACC_RAIN_COUNTL, "--")) {
				value = Float.valueOf(dto.BUCKET_ACC_RAIN_COUNTL);
			}
			dto.y = chartH-chartH*Math.abs(value)/(Math.abs(maxValue)+Math.abs(minValue)) + topMargin;
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.BUCKET_ACC_RAIN_COUNT) && !TextUtils.equals(dto.BUCKET_ACC_RAIN_COUNT, "--")) {
				float value = Float.valueOf(dto.BUCKET_ACC_RAIN_COUNT);
				if (value != 0) {
					float BUCKET_ACC_RAIN_COUNTWidth = textP.measureText(dto.BUCKET_ACC_RAIN_COUNT+"");
					canvas.drawText(dto.BUCKET_ACC_RAIN_COUNT+"", dto.x-BUCKET_ACC_RAIN_COUNTWidth/2, dto.y-(int)CommonUtil.dip2px(mContext, 5), textP);
				}
			}
		}
		...
		for (int i = 0; i < size; i++) {
			...
			//绘制曲线上每个点的数据值
			if (!TextUtils.isEmpty(dto.BUCKET_ACC_RAIN_COUNTL) && !TextUtils.equals(dto.BUCKET_ACC_RAIN_COUNTL, "--")) {
				float value = Float.valueOf(dto.BUCKET_ACC_RAIN_COUNTL);
				if (value != 0) {
					float BUCKET_ACC_RAIN_COUNTWidth = textP.measureText(dto.BUCKET_ACC_RAIN_COUNTL+"");
					canvas.drawText(dto.BUCKET_ACC_RAIN_COUNTL+"", dto.x-BUCKET_ACC_RAIN_COUNTWidth/2, dto.y+(int)CommonUtil.dip2px(mContext, 10), textP);
				}
			}
		}
