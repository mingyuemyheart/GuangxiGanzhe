package com.cxwl.shawn.guangxi.ganzhe.manager;
	...
	private class LoadThread extends Thread {
		...
		public void run() {
			...
			for (int i = 0; i < len ; i++) {
				...
				loadImage(i, radar.time0, radar.imgUrl, radars);
			}
		}
		
		private void loadImage(final int index, final String time0, final String url, final List<RadarDto> radars) {
			new Thread(new Runnable() {
				@Override
				public void run() {
					String path = decodeFromUrl(url, time0, index);//图片下载后存放的路径
					if (!TextUtils.isEmpty(path)) {
						radars.get(index).imgPath = path;
					}
		...
		private String decodeFromUrl(String url, String time0, int index){
		    try {
				...
				try {
					File file = new File(getDir() + "/"+time0+index+".png");
